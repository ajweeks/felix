...
use std::{
    collections::{btree_map::Entry, HashMap, BTreeMap},
    ffi::CString,
    path::PathBuf,
    sync::Arc,
};
...
pub struct ShaderPipelineCommon {
    ...
    pub set_layout_info: Vec<BTreeMap<u32, vk::DescriptorType>>,
...
pub fn create_descriptor_set_layouts(
    ...
    Vec<BTreeMap<u32, vk::DescriptorType>>,
) {
    // dbg!(&descriptor_sets);
            ...
            for (binding_index, binding) in set.iter() {
                match binding.ty {
                    ...
                    rspirv_reflect::DescriptorType::SAMPLED_IMAGE => {
                        if binding.binding_count == rspirv_reflect::BindingCount::unbounded {
                            // Bindless
...
pub fn create_compute_pipeline(
    ...
    let (descriptor_set_layouts, set_layout_info) = create_descriptor_set_layouts(
        device,
